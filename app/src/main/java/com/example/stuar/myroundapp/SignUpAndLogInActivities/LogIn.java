...
public class LogIn extends AppCompatActivity {
    ...
    private void AllowAccessToAccount(final String email, final String password) {
        if(checkBox.isChecked()) {
            Paper.book().write(RememberMe.UserEmailKey, email);
        ...
        RootRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.child(parentDbName).child(email).exists()) {
                    ...
                    Toast.makeText(LogIn.this, "Account with this " + email + " number do not exists.", Toast.LENGTH_SHORT).show();
                    loadingBar.dismiss();
                }
            }
    ...
    public void onSignUpLinkBtnClick(View view){
        if(view.getId() == R.id.signUpLinkBtn){
            finish();
            startActivity(new Intent(LogIn.this, CustomerSignUpActivity.class));
        }
    }
}